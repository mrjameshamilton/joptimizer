...
/**
 * ClassOptimizer is responsible for optimizing the bytecode of a class model
 * using a series of optimization rules. It supports a variety of configurable
 * optimizations and tracks statistics related to the optimization process.
 * <p>
 * The main purpose of this class is to process and transform the bytecode of
 * a given class while preserving its functional behavior, reducing the size
 * of the class, or making it more computationally efficient. This is achieved
 * by applying multiple optimization strategies, selectively controlling the
 * methods and their execution patterns.
 */
public class ClassOptimizer {
     ...
     * class hierarchy resolver, and bytecode data.
     * It parses the provided byte array into a class model to be optimized.
     *
     * @param stats the optimization statistics object used to record metrics and track optimization progress
    ...
    public ClassOptimizer(OptimizationStats stats, ClassHierarchyResolver resolver, byte[] bytes) {
        ...
        this.original = ClassFile.of(KEEP_DEAD_CODE, ClassHierarchyResolverOption.of(resolver)).parse(bytes);
    }

    /**
     * Optimizes the bytecode using the provided list of {@link Optimization} instances.
     *
     * @param optimizations a variable number of {@link Optimization} instances to apply
    ...
    /**
     * Finds the ending index in a list of code elements based on a starting index and the number of executable
     * elements to skip.
     *
    ...
    /**
     * Determines if the provided {@code CodeElement} is an executable element.
     * An element is considered executable if it is an instance of {@code Instruction}, {@code Label},
     * or {@code ExceptionCatch}.
     *
    ...
    private Optional<byte[]> generateOptimizedBytes(List<Optimization> optimizations) {
        try {
            return Optional.of(ClassFile.of(KEEP_DEAD_CODE, ClassHierarchyResolverOption.of(resolver))
                .transformClass(original,
                    transformingMethods(
                        (methodBuilder, method) -> {
                            if (method instanceof CodeAttribute codeModel) {
                                methodBuilder.withCode(codeBuilder -> {
                                    processCodeOptimization(optimizations, codeModel, codeBuilder);
                                });
                            } else {
    ...
    private void processCodeOptimization(List<Optimization> optimizations, CodeAttribute code, CodeBuilder codeBuilder) {
        ...
        var executableElements = elements.stream()
            .filter(ClassOptimizer::isExecutableElement)
            .toList();
    ...
    private void processCodeElements(List<CodeElement> elements, List<CodeElement> executableElements,
                                     List<Optimization> optimizations, CodeBuilder codeBuilder) {
