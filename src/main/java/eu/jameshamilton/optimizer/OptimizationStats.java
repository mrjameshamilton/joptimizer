...
public class OptimizationStats {
    ...
    public void printPassSummary() {
        ...
        logger.info("Pass {}: classes={} ok={} err={} classesOptimized={} bytesReduced={} avgBytesReduced={}",
            currentPass, files, ok, err, classes, bytes, avg);
    }
    ...
    public void printSummary() {
        ...
        applicationCountByPass.forEach((pass, attemptMap) -> {
            ...
            String header = String.format("%-" + nameColWidth + "s | %" + countColWidth + "s", "Optimization", "Removed");
            String sep = "-".repeat(Math.max(title.length(), header.length()));

            sb.append(title).append('\n');
            sb.append(sep).append('\n');
            sb.append(header).append('\n');
            sb.append("-".repeat(nameColWidth)).append("-+-").append("-".repeat(countColWidth)).append('\n');

            attemptMap.forEach((name, attempts) -> {
                long removedInstructions = passSuccesses
                    .getOrDefault(pass, Map.of())
                    .getOrDefault(name, new java.util.concurrent.atomic.LongAdder())
                    .sum();

                String trimmed = name.length() > 30 ? name.substring(0, 30) : name;
                sb.append(String.format("%-" + nameColWidth + "s | %" + countColWidth + "d", trimmed, removedInstructions))
                  .append('\n');
            });

            long bytesReduced = totalBytesReduced.getOrDefault(pass, new java.util.concurrent.atomic.LongAdder()).sum();
            long optimizedClasses = optimizedClassCount.getOrDefault(pass, new java.util.concurrent.atomic.LongAdder()).sum();
            if (optimizedClasses > 0) {
                sb.append(sep).append('\n');
                sb.append(String.format("Optimized %d classes, reduced %d bytes (avg %.2f bytes/class)",
