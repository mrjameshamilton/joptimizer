...
class OptimizationTest {
    ...
    public void testNopRemoval() {
        given(resolver, code -> code.nop().nop().return_())
                .when(classModel -> optimize(classModel, new NopRemover()))
                .expect(code -> code.return_());
    }
    ...
    public void testStringToString() {
        given(
                        resolver,
                        code ->
                                code.loadConstant("string")
                                        .invokevirtual(
                                                of("java.lang.String"),
                                                "toString",
                                                ofDescriptor("()Ljava/lang/String;"))
                                        .return_())
                .when(classModel -> optimize(classModel, new ConstantToStringOptimization()))
                .expect(code -> code.loadConstant("string").return_());
    }
    ...
    public void multipleByOne() {
        given(
                        resolver,
                        code -> {
                            code.loadConstant(5).istore(0).iload(0).loadConstant(1).imul();
                        })
                .when(classModel -> optimize(classModel, new MultiplyByOne()))
                .expect(code -> code.loadConstant(5).istore(0).iload(0));
    }
    ...
    public void normalizeConstantAddCombine() {
        given(
                ...
                .expect(code -> code.iconst_0().istore(1).loadConstant(-1).iload(1).isub());
    }
    ...
    public void constantStringSubstring1() {
        given(
                ...
                .expect(code -> code.loadConstant("World"));
    }
    ...
    public void constantStringSubstring2() {
        given(
                ...
                .when(code -> optimize(code, new ConstantStringSubstring()))
                .expect(code -> code.loadConstant("Hello"));
    }
    ...
    public void constantStringToString() {
        given(
                ...
                .expect(code -> code.loadConstant("Hello World"));
    }
    ...
    public void constantStringEquals() {
        given(
                ...
                .expect(code -> code.loadConstant(1));
    }
                                ...
                                code.loadConstant("Hello World")
                                        ...
                                        .invokevirtual(
                                                ...
                                                MethodTypeDesc.ofDescriptor(
                                                        "(Ljava/lang/String;)Z")))
                .when(code -> optimize(code, new ConstantStringEquals()))
                .expect(code -> code.loadConstant(0));
    }

    private ClassModel optimize(ClassModel classModel, Optimization... optimizations) {
        byte[] inputBytes =
                ClassFile.of(KEEP_DEAD_CODE).transformClass(classModel, ClassFileBuilder::with);
        byte[] bytes =
                new ClassOptimizer(stats, resolver, inputBytes).optimize(optimizations).get();
